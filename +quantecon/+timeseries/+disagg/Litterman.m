function result = Litterman(Y, x, opts)
% Litterman  Temporal disaggregation via the Litterman (1983) method
%
%   Random-walk variant of Chow-Lin. The error structure is
%   (I + rho*L)(I + L) e_t = a_t, i.e., differenced AR(1).
%   Rho estimated by grid search (WLS or ML).
%
% SYNTAX:
%   result = quantecon.timeseries.disagg.Litterman(Y, x)
%   result = quantecon.timeseries.disagg.Litterman(Y, x, Freq=3)
%
% INPUT:
...
%   Rho       - [] | scalar | [rmin rmax ngrid]  (default [])
%
% OUTPUT:  same structure as ChowLin
%
% REFERENCE:
%   Litterman, R.B. (1983) "A random walk, Markov model for the
%   distribution of time series", J. Business & Economic Statistics,
%   1(2), 169-173.
%
% Source: Refactored from EMQuilis Temporal Disaggregation Toolbox v2.1.
...
sc   = opts.Freq;
type = double(opts.Method == "ml");
N    = length(Y);
[n, p] = size(x);

% --- Intercept pretest ---
if opts.Intercept == "auto"
    pre = lit_core(Y, x, ta, sc, type, true, opts.Rho, N, n, p);
...
res = lit_core(Y, x, ta, sc, type, addInt, opts.Rho, N, n, p);
...
function res = lit_core(Y, x, ta, sc, type, addInt, rl, N, n, p)
    ...
    nrl = numel(rl);
    if nrl == 0
        rho = grid_rho_lit(Y, X, C, N, n, type, [0.05 0.99 50]);
    elseif nrl == 1
        rho = rl;
    elseif nrl == 3
        rho = grid_rho_lit(Y, X, C, N, n, type, rl);
    else
        error('Litterman:BadRho','Rho must be [], scalar, or [rmin rmax ngrid].');
    end

    In = eye(n);
    LL = diag(-ones(n-1,1), -1);
    % Key difference from Chow-Lin: Aux = (I + rho*LL)*(I + LL)
    Aux = (In + rho * LL) * (In + LL);
...
function rho = grid_rho_lit(Y, X, C, N, n, type, rl)
